...
@Component({
  ...
  <div class="keg-form">
    ...
    <button (click)="addKeg(newName, newBrewery, newPrice, newABV)" class="btn btn-info btn-sm">Tap</button>
  </div>
  `
})
export class NewKegComponent {
  ...
  addKeg(tapName: HTMLInputElement, tapBrewery: HTMLInputElement, tapPrice: HTMLInputElement, tapABV: HTMLInputElement){
    var newKeg = new Keg(tapName.value, tapBrewery.value, tapPrice.value, tapABV.value, 124);
